...
void SFMLDebugDraw::DrawPolygon(const b2Vec2 *vertices, int32 vertexCount,
  ...
  for (int i = 0; i < vertexCount; i++) {
    ...
    polygon.setPoint(
        i, sf::Vector2f(std::floor(transformedVec.x),
                        std::floor(transformedVec.y))); // flooring the coords
                                                        // to fix distorted
                                                        // lines on flat
                                                        // surfaces
...
void SFMLDebugDraw::DrawSolidPolygon(const b2Vec2 *vertices, int32 vertexCount,
                                     const b2Color &color) {
  sf::ConvexShape polygon(vertexCount);
  for (int i = 0; i < vertexCount; i++) {
    // polygon.setPoint(i, SFMLDraw::B2VecToSFVec(vertices[i]));
...
void SFMLDebugDraw::DrawCircle(const b2Vec2 &center, float32 radius,
                               const b2Color &color) {
...
void SFMLDebugDraw::DrawSolidCircle(const b2Vec2 &center, float32 radius,
  ...
  sf::Vertex line[2] = {
      sf::Vertex(SFMLDebugDraw::B2VecToSFVec(center),
                 SFMLDebugDraw::GLColorToSFML(color)),
      sf::Vertex(SFMLDebugDraw::B2VecToSFVec(endPoint),
                 SFMLDebugDraw::GLColorToSFML(color)),
  };
...
void SFMLDebugDraw::DrawSegment(const b2Vec2 &p1, const b2Vec2 &p2,
                                const b2Color &color) {
  sf::Vertex line[] = {sf::Vertex(SFMLDebugDraw::B2VecToSFVec(p1),
                                  SFMLDebugDraw::GLColorToSFML(color)),
                       sf::Vertex(SFMLDebugDraw::B2VecToSFVec(p2),
                                  SFMLDebugDraw::GLColorToSFML(color))};
...
void SFMLDebugDraw::DrawTransform(const b2Transform &xf) {
  ...
  // in Box2D testbed
  // That's because the ordinate in SFML coordinate system points downward while
  // the OpenGL(testbed) points upward
